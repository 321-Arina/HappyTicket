...
public class Algorithm {
    ...
    public boolean isHappyTicket(String input) {
        ...
        for(int i = 0; i < 3; i++) { // цикл заполнения сумм чётных и нечётных цифр
            s1 = s1 + inputInt % 10;
    ...
    public int nextHappyTicketV2(String input) {
        ...
        for (int i = inputInt; i < 1_000_000; i++) { // цикл проверки и поиски счастливого билета
            int s1 = i%10 + (i/100)%10 + (i/10000)%10; // подсчёт суммы чётных чисел
            int s2 = (i/10)%10 + (i/1000)%10 + (i/100000)%10; // подсчёт суммы нечётных чисел
